...
chip::app::StatusIB ConvertToStatusIB(CHIP_ERROR err)
{
    ...
    {
        return chip::app::StatusIB(Status::Failure, chip::to_underlying(OperationalCertStatus::kInvalidPublicKey));
    }
    else if (CHIP_ERROR_WRONG_NODE_ID == err)
    {
        return chip::app::StatusIB(Status::Failure, chip::to_underlying(OperationalCertStatus::kInvalidNodeOpId));
    }
    else if (CHIP_ERROR_UNSUPPORTED_CERT_FORMAT == err)
    {
        return chip::app::StatusIB(Status::Failure, chip::to_underlying(OperationalCertStatus::kInvalidNOC));
    }
    else if (CHIP_ERROR_FABRIC_EXISTS == err)
    {
        return chip::app::StatusIB(Status::Failure, chip::to_underlying(OperationalCertStatus::kFabricConflict));
    }
    else if (CHIP_ERROR_INVALID_FABRIC_ID == err)
    {
        return chip::app::StatusIB(Status::Failure, chip::to_underlying(OperationalCertStatus::kInvalidFabricIndex));
    }
    else
    {
        return chip::app::StatusIB(err);
    }
}
